...
==========================================================
Intermediate signal model: multiple receive antennas but no per-user fading.
All users share a known fixed spatial signature g = 1 ∈ R^M.

Signal model:
    Y = (sum_b P_b C_b^T a_b) 1^T + Z       ∈ R^{n × M}  (or C)
  ...
  - M receive antennas, but no per-user fading: every user's spatial
    signature is the known constant g = 1.  The noiseless signal is
    the rank-1 matrix y_clean @ g^T where y_clean is identical to V1.
  - Z is i.i.d. AWGN of shape (n, M).

Decoder target:
    message_counts ∈ Z_+^{num_codewords}, still directly aligned with
...
def synthesize_received_signal(P_mats: dict[int, np.ndarray], block_dicts: dict[int, np.ndarray], block_coeffs: dict[int, np.ndarray], M: int, noise_var: float, rng: np.random.Generator, complex_valued: bool = False) -> tuple[np.ndarray, np.ndarray]:
    """Synthesize Y = (sum_b P_b C_b^T a_b) g^T + Z  with g = 1 ∈ R^M.

    The noiseless signal is rank-1: y_clean (n,) replicated across M antennas.
    ...
    for b in P_mats:
        ...
        if np.any(a_b):
            ...
            x_b = C_b.T @ a_b                  # (d,)
            y_clean += P_mats[b] @ x_b         # (n,)

    g = np.ones(M, dtype=dtype)                # shared spatial signature
    Y_clean = np.outer(y_clean, g)             # (n, M)
...
def print_diagnostics(codebook, P_mats, msg_to_block, block_to_msg_list, active_msgs, message_counts, block_coeffs, Y_clean, Y_noisy, noise_var, esn0_db):
    ...
    print("\n" + "=" * 60)
    print("ODMA + URA Simulation — V2 (multi-antenna, no fading, g=1)")
    print("=" * 60)
    ...
    print(f"Spatial signature : g = 1 ∈ R^{M}")
...
def main():
    parser = argparse.ArgumentParser(description="ODMA + URA scaffold — V2 (multi-antenna, no fading)")
