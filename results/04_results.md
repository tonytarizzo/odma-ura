...
**Scope:** verified framework results and completed `018--026` HPC audit. Jobs `024--026` are artifact-complete; job `023`
returned 16 of 26 array rows and is treated as a partial experiment rather than silently completed.
...
16. The next experiment should be a small, preregistered decoder-calibration and association diagnostic, not another broad encoder sweep. The gates are non-saturated local evidence, SNR-responsive support recall, and a justified outer-association benefit before resubmitting the full `B=128` bank.

The present research conclusion and next question are therefore:

> The section backend itself is exact, scalable, and energy-valid. The remaining scalable bottlenecks are local-evidence
> calibration at large section alphabets and complete-message association under high local occupancy. The next work should
> fix and certify those two decoder stages before widening the outer-code or encoder search.
...
The manifest contains 72 rows: 16 sparse support sizes, dense and ODMA controls, two seeds, and D0/D1. All 72 per-array
logs returned, but only 68 tasks wrote both `summary.json` and `checkpoint.pt`.
...
Initialisation now resamples exact zeros. The four rows must still be rerun; no checkpoint was written before the
exception.

The 68 complete artifacts pass the substantive audit:
...
- bit-identical encoder states and identical matched-filter streams in all 34 returned D0/D1 pairs;
- exact nested support/sign inclusion across every returned support within both seeds;
- maximum unit-energy deviation `9.54e-7`;
- no other traceback among the 72 logs.

The strict merger therefore refuses the current tree. The tables below use the 68-summary `--allow-incomplete`
aggregate. Four-decimal completed log output supplies only the two-seed full-density context; it is clearly marked and is
not inserted into the strict JSON/TSV aggregate.
...
|---|---:|---:|---:|---|
| dense | 1 | 0.2935 | 0.2239 | approximate two-seed mean; second seed from 4-decimal logs |
| sparse global `s=256` | 1 | 0.2979 | 0.2226 | approximate two-seed mean; second seed from 4-decimal logs |
...
Three conclusions survive the incomplete controls.

First, the no-loss region is broad. Relative to the approximate two-seed dense context, D0 at `s=48` differs by only
`+0.0039`, and `s=32` by `+0.0115`. D1 at `s=16` differs by only `+0.0084`. The D1 curve is noisier: its seed standard
error reaches about `0.013` at `s=16`, and its mean five-epoch tail slope remains negative after 20 epochs. Its exact
transition is therefore training-budget and replication dependent; the defensible claim is a band around `s=8--24`,
not that `s=16` is optimal.
...
1. rerun only array indices `3,4,67,68`, then require the strict merger to pass;
2. increase replication/evaluation only at `s=8,16,32,64` to locate the transition reliably;
3. repeat those few supports at `B=14,n=256` to test absolute-support versus density scaling;
4. compare a procedural support family against those explicit controls using both recovery and the geometry table above;
5. keep the sectioned large-`B` calibration/association branch separate until it clears its own local-evidence gates.
